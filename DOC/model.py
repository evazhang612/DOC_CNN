...
class RNNModel(nn.Module):
    ...
    def __init__(self, rnn_type, vocab, ntoken, ninp, nhid, nhidlast, nlayers, 
        ...
        self.model_embeddings_target = ModelEmbeddings(ninp, vocab.tgt)
    ...
    def forward(self, input, hidden, return_h=False, return_prob=False):
        ...
        #EVA 
        # source_padded_chars = self.vocab.src.to_input_tensor_char(self.source) 

        #ENCODE HERE @EVA 
        X = self.model_embeddings_source(input)
        X_packed = pack_padded_sequence(X, source_lengths)
        input = X_packed # geenerate
